...
(async function () {
    ...
    async function handleSetlists(jsonData) {
        ...
        const setlistNameInput = await waitForElementAsync("input[placeholder='Enter Setlist Name']", iframeDocument);
        if (setlistNameInput) {
    ...
    async function handleAddSong(song, index) {
        ...
        if(index == 0){
            // CLick on Add Song
            const addSongButton = await waitForElementAsync("div.setlists-details--add-message div a"); // Adjusted selector
            addSongButton.click();
        }
        ...
        const searchInput = await waitForElementAsync("input.setlists-details--song-search--input.js-add-item-song-search-input");
        if (searchInput) {
            searchInput.focus();
        ...
        const selector = song.type === "song" ? "a.js-add-song-save" : song.type === "pad" ? ".js-add-pad-save" : null;
        if (!selector) return console.error("Unknown type: ", song.type);

        const addButton = await waitForElementAsync(selector);
        addButton ? addButton.click() : console.error(`Add button for ${song.type} not found.`);
        await new Promise((resolve) => setTimeout(resolve, 200)); // Wait
    ...
    async function selectKey(key) {
        ...
        if (keyButton) {
            ...
            console.warn(`Key button not found for base key: "${key[0].toUpperCase()}"`);
            return;
        }
    ...
    async function handleSongTempo(song) {
        if (song.tempo && song.tempo !== "") {
            // Click the switch
            const switchElement = document.querySelector('div:nth-child(4) > .onoff-switch > .onoff-switch--label > .onoff-switch--inner');
            if (switchElement) {
                switchElement.click();
            } else {
    ...
    async function clickSongSpanByText(targetText) {
        ...
        while (Date.now() < timeout) {
            ...
            for (const span of spans) {
                if (span.textContent.trim().toLowerCase() === targetText.toLowerCase().trim()) {
    ...
    async function saveSetlist() {
        const saveToCloudButton = await waitForElementAsync("[role='link'][name='Save to Cloud']");
    ...
    async function routePage() {
        ...
        if (window.location.href.includes('https://immanuel-detmold.church.tools/?q=churchservice')) {
            addPlaylistButton();
            await new Promise((resolve) => setTimeout(resolve, 1000)); // Wait 1 second
        } else if (
            window.location.href.includes("https://www.multitracks.com/premium/setlists/?setlistSelection=") ||
            ...
            try {
                ...
                alert("Fehler beim Lesen der Zwischenablage oder JSON-Daten: " + error.message);
                console.error("Clipboard or JSON error:", error);
            }
        } else if (window.location.href.includes("https://www.multitracks.com/premium/setlists/details.aspx?setlistID")) {
            if (clipboardData) {
                try {
                    const jsonData = JSON.parse(clipboardData);
    ...
    function addPlaylistButton() {
        waitForElementAsync('#cdb_group form').then((form) => {
            ...
            button.type = 'button';
            button.innerHTML = '<i class="fas fa-music fa-fw" aria-hidden="true"></i> Multitracks Playlist';
