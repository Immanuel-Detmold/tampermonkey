...
(async function () {
  ...
  async function clickTabBasedOnType(type) {
      ...
      if (type === "song") {
          tab = await waitForElementAsync("li[data-container='library'] a.tab-filter");
      } else if (type === "pad") {
  ...
  ['load', 'popstate', 'hashchange'].forEach(event => window.addEventListener(event, addPlaylistButton));

  // Observe DOM changes to handle dynamic content
  new MutationObserver(addPlaylistButton).observe(document.body, { childList: true, subtree: true });
